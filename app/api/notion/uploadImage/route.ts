...
export async function POST(req: NextRequest) {
  ...
  let bucket: any = "cover";
  if (req.nextUrl.searchParams.get("bucket") !== null) {
  ...
  const { error } = await supabase.storage
    .from(bucket)
    .upload(
      `${bucket}-form-${req.nextUrl.searchParams.get("id")}.webp`,
  ...
  const publicURL = supabase.storage
    .from(bucket)
    .getPublicUrl(`${bucket}-form-${req.nextUrl.searchParams.get("id")}.webp`);
